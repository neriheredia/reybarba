...
const SearchBar = <T extends SearchBarListItem>({
  theme = "dark",
  placeholder,
  options,
  renderItem,
  ...
  const iconColor = useMemo(
    () => (theme === "dark" ? colors.darkGray : "white"),
    [theme]
  );
  const backgroundColorTextInput = useMemo(
    () => (theme === "dark" ? "white" : "rgba(255, 255, 255, .1)"),
    [theme]
  );

  useEffect(() => {
    const auxFiltered = options.filter((opt) =>
      new RegExp(text, "i").test(opt.label)
    );
