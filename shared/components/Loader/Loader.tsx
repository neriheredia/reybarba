...
import { View, Animated, Text, Modal, StatusBar } from 'react-native';
...
const Loader = ({show = false, title, subtitle, background = "rgba(43, 43, 43,.8)"} : LoaderProps) => {
    ...
    const calculateTranslateFor = (type : tranlateTypes) => {
        ...
        const snapshot = 50, radius = moderateScale(24,1.1);

        const inputRange = [], outputRange = [];
        for (var i=0; i<=snapshot; ++i) {
            const value = i/snapshot;
            const move = (type === "x" ? (Math.sin(value * Math.PI * 2)) : (-Math.cos(value * Math.PI * 2)))  * radius;
    ...
    return(
        // <Modal  transparent animationType='slide'>
            // <StatusBar backgroundColor="rgba(0, 0, 0, .7)" barStyle="dark-content"/>
            <View style={[styles.container, { backgroundColor: background }]}>
                <View style={{backgroundColor: (title || subtitle ) ? colors.white : "transparent", paddingVertical: verticalModerateScale(52,1.1), borderRadius: 20}}>
                    <View style={{position: "absolute", top: -moderateScale(92,1.1)/2, width: moderateScale(92,1.1), height: moderateScale(92,1.1), borderRadius: moderateScale(92,1.1), backgroundColor: (title || subtitle ) ? colors.white : "transparent", alignSelf:"center"}}>
                        ...
                        <Animated.View style={[styles.ball,styles.ball2,{
                            transform: [
                            {
                        ...
                        <Animated.View style={[styles.ball, styles.ball3,{
                            transform: [
                            {
                        ...
                        <Animated.View style={[styles.ball, styles.ball4,{
                            transform: [
                            {
                        ...
                        <Animated.View style={[styles.ball, styles.ball5,{
                            transform: [
                            {
